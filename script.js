...
const profesi=['DESIGN ARCHITECTURE', 'CONTRACTOR', 'BUILDING',
                ];
...
function typeEffect() {
    const curProfesi = profesi
    [indexProfesi]
    const curChar    = curProfesi.
    substring(0, indexChar);
    ...
    if(!hapus && indexChar < curProfesi.length) {
        ...
        hapus =  !hapus;
        typing.classList.remove ('stop-bling');
        indexProfesi = !hapus ? (indexProfesi + 1) % profesi.
        length : indexProfesi;
...
toggleMenu.onclick = () => {
    ...
    document.querySelector('.navbar .nav-menu i').
    classList.toggle('active');
